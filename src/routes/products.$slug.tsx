...
import { CASE_SIZE, JAR_SIZE, productBySlug, products } from "@/lib/content";
...
export const Route = createFileRoute("/products/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      {
        title: loaderData
          ? `${loaderData.product.danish} — Dansk Lyng`
          : "Dansk Lyng",
      },
    ],
  }),
        ...
        <div className="flex flex-col justify-center px-4 py-16 sm:px-12">
          ...
          <dl className="mt-10 grid max-w-md grid-cols-2 gap-6 text-sm">
            ...
            <div>
              ...
              <dd className="text-ink-soft">{t.caseSize}</dd>
            </div>
          ...
          <p className="mt-8 text-sm text-muted">
            {JAR_SIZE} · {CASE_SIZE} {lang === "zh" ? "罐／箱" : "jars / case"}
          </p>
          <div className="mt-10 flex flex-wrap gap-4">
            <SolidLink to="/partner/apply">{t.requestSpec}</SolidLink>
            <Link to="/products" className="inline-flex min-h-11 items-center text-sm text-ink-soft">
              {t.backProducts}
            </Link>
          </div>
        </div>
...
function NotFoundProduct() {
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-24 text-center">
      ...
      <Link to="/products" className="mt-6 inline-flex min-h-11 items-center text-sm">
        {t.backProducts}
      </Link>
    </div>
  );
